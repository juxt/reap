...
(defn- langtag [_]
  (p/into
   {:juxt.reap.rfc5646/lang-type :langtag}
   (p/pattern-parser
    (re-pattern
     (str
      "(?<Language>"
...
(defn- privateuse [_]
  (p/into
   {:juxt.reap.rfc5646/lang-type :privateuse}
   (p/pattern-parser
    #"x-(\p{Alnum}{1,8}(?:-\p{Alnum}{1,8})*)(?!\p{Alnum})"
    {:group
     {:juxt.reap.rfc5646/langtag 0
      :juxt.reap.rfc5646/privatetag 1}})))
...
(defn- irregular [opts]
  (p/into
   {:juxt.reap.rfc5646/lang-subtype :irregular}
   (match-literal
    #{"en-GB-oed"
      "i-ami"
      "i-bnn"
      ...
      "sgn-BE-NL"
      "sgn-CH-DE"}
    (conj opts [:group {:juxt.reap.rfc5646/langtag 0}]))))

(defn- regular [opts]
  (p/into
   {:juxt.reap.rfc5646/lang-subtype :regular}
   (match-literal
    #{"art-lojban"
      "cel-gaulish"
      "no-bok"
...
(defn- grandfathered [opts]
  (p/into
   {:juxt.reap.rfc5646/lang-type :grandfathered}
   (p/alternatives
    (irregular opts)
    (regular opts))))
